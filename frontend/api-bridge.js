...
// Safe Global Variable Initializations
window.currentUser = window.currentUser || null;
window.currentReading = window.currentReading || null;
window.currentMasterReport = window.currentMasterReport || null;
window.imageToAnalyze = window.imageToAnalyze || null;
window.selectedCards = window.selectedCards || [];
window.masterDrawnCards = window.masterDrawnCards || [];
window.activeSpreadMode = window.activeSpreadMode || 'three';
window.consultantChartObj = window.consultantChartObj || null;
window.readerChartObj = window.readerChartObj || null;
window.radarChartObj = window.radarChartObj || null;
...
async function syncRemoteData() {
  ...
  if (typeof updateRegisteredProfilesSidebar === 'function') updateRegisteredProfilesSidebar();
  if (typeof updateDashboardData === 'function') updateDashboardData();
  if (typeof renderRecommendationsList === 'function') renderRecommendationsList();
  if (typeof renderHistoryTable === 'function') renderHistoryTable();
  if (typeof updateNotificationBadge === 'function') updateNotificationBadge();
  return me;
}

if (typeof window.AetheriaDatabaseManager === 'undefined') {
  window.AetheriaDatabaseManager = {};
}
...
window.onload = async function () {
  ...
  const token = localStorage.getItem(AETHERIA_TOKEN_KEY);
  if (!token) { if (typeof navigate === 'function') navigate('home'); return; }
  try { await syncRemoteData(); if (typeof initializeUserWorkspace === 'function') initializeUserWorkspace(); }
  catch (_) { localStorage.removeItem(AETHERIA_TOKEN_KEY); localStorage.removeItem('aetheria_session'); if (typeof navigate === 'function') navigate('home'); }
};
...
window.handleLoginSubmit = async function (e) {
  ...
  try {
    ...
    if (typeof initializeUserWorkspace === 'function') initializeUserWorkspace();
  } catch (err) {
...
window.handleRegisterSubmit = async function (e) {
  ...
  try {
    ...
    if (document.getElementById('register-wizard-form')) document.getElementById('register-wizard-form').reset(); 
    if (typeof moveWizard === 'function') moveWizard(1); 
...
window.logout = function () {
  ...
  currentUser = null; if (typeof navigate === 'function') navigate('home');
};

window.runPalmCVAnalysis = async function () {
  if (!imageToAnalyze) return;
  const loading = document.getElementById('cv-loading-overlay'); 
  const idle = document.getElementById('cv-idle-overlay');
  ...
  try {
    ...
    img.onload = () => { 
      ...
      if (canvas) {
        ...
        if (typeof renderPalmLinesOnCanvas === 'function') renderPalmLinesOnCanvas(lastAnalysisMetrics); 
      }
    };
    img.src = record.image;
    const setTxt = (id, val) => { const el = document.getElementById(id); if (el) el.innerText = val; };
    setTxt('cv-val-heart', `${row.heart_line_conf}%`); setTxt('cv-val-head', `${row.head_line_conf}%`); setTxt('cv-val-life', `${row.life_line_conf}%`); setTxt('cv-val-fate', `${row.fate_line_conf}%`); setTxt('cv-val-sun', `${row.sun_line_conf}%`);
    const existing = typeof getReadings === 'function' ? getReadings() : [];
    localStorage.setItem('aetheria_readings', JSON.stringify([record, ...existing]));
    currentReading = record; await syncRemoteData(); if (typeof showPalmResult === 'function') showPalmResult(record);
  } catch (err) { alert(err.message); if (idle) idle.classList.remove('hidden'); }
...
window.submitTarotInterpretation = async function () {
  ...
  const focus = document.getElementById('tarot-focus-intent')?.value || 'General';
  try {
    ...
    const record = toTarotRecord(row); currentReading = record; await syncRemoteData(); if (typeof showTarotResult === 'function') showTarotResult(record);
  } catch (err) { alert(err.message); }
};
...
window.saveProfileChanges = async function () {
  ...
  try { await aetheriaApi('/users/me', { method: 'PUT', body: JSON.stringify(payload) }); await syncRemoteData(); if (typeof updateProfileView === 'function') updateProfileView(); alert('Profile preferences updated.'); }
  catch (err) { alert(err.message); }
};
...
window.updateAdminConsole = async function () {
  try {
    ...
    set('admin-kpi-users', stats.total_users); set('admin-kpi-readings', stats.total_readings);
    set('admin-kpi-cv', stats.palm_readings ? `${Math.round((stats.palm_readings / Math.max(stats.total_readings, 1)) * 100)}%` : '0%');
    set('admin-kpi-latency', stats.average_api_latency_ms == null ? '—' : `${stats.average_api_latency_ms} ms`);
  ...
  } catch (err) { if (typeof addAuditLog === 'function') addAuditLog(`Admin analytics unavailable: ${err.message}`); }
};
...
window.updateReaderConsole = async function () {
  ...
  } catch (err) { if (typeof addAuditLog === 'function') addAuditLog(`Reader analytics unavailable: ${err.message}`); }
};
...
window.updateConsultantConsole = async function () {
  try {
    ...
    const set = (id, val) => { const el = document.getElementById(id); if (el) el.innerText = val; };
    set('consultant-kpi-sessions', `${data.reading_count || 0} Sessions`);
    set('consultant-kpi-score', data.average_guidance_score == null ? '—' : `${data.average_guidance_score}/100`);
    set('consultant-kpi-retention', `${Number(data.repeat_client_rate || 0).toFixed(1)}%`);
    set('consultant-kpi-active-days', data.active_days_30d ?? 0);
    set('consultant-kpi-reports', data.report_ready_count ?? 0);
    set('consultant-kpi-clients', data.client_count || 0);
  ...
  } catch (err) { if (typeof addAuditLog === 'function') addAuditLog(`Consultant analytics unavailable: ${err.message}`); }
};
...
window.handleCombinedPalmUpload = function (event) {
  ...
  const reader = new FileReader(); reader.onload = () => { combinedPalmImageBase64 = reader.result; window._combinedPalmReadingId = null; const prompt=document.getElementById('combined-palm-upload-prompt'); const container=document.getElementById('combined-palm-canvas-container'); if(prompt) prompt.classList.add('hidden'); if(container) container.classList.remove('hidden'); const canvas=document.getElementById('combinedPalmCanvas'); if(canvas){ const img=new Image(); img.onload=()=>{const ctx=canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0,canvas.width,canvas.height);}; img.src=reader.result; }}; reader.readAsDataURL(file);
};

window.initCombinedWizard = function () { masterDrawnCards = []; if (typeof renderCombinedDrawnCardsGrid === 'function') renderCombinedDrawnCardsGrid(); useLatestPalmScan(); };
...
window.redrawCombinedTarotSpread = function () {
  ...
  const pool = typeof TAROT_CARDS !== 'undefined' ? [...TAROT_CARDS] : [];
  selectedCards = [];
  ...
  if (typeof renderCombinedDrawnCardsGrid === 'function') renderCombinedDrawnCardsGrid();
};
...
window.executeCombinedMasterAnalysis = async function () {
  ...
  try {
    ...
    masterDrawnCards = currentMasterReport.tarotCards; 
    if (typeof renderCombinedDrawnCardsGrid === 'function') renderCombinedDrawnCardsGrid(); 
    if (typeof renderMasterReportUI === 'function') renderMasterReportUI(currentMasterReport); 
    await syncRemoteData();
    const resultContainer=document.getElementById('view-combined-result'); 
    if(resultContainer){resultContainer.classList.remove('hidden'); resultContainer.scrollIntoView({behavior:'smooth'});}
  } catch (err) { alert(err.message); }
};
...
window.renderMasterReportUI = function (report) {
  ...
  if(canvas && path){ const img=new Image(); img.onload=()=>{const ctx=canvas.getContext('2d'); ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0,canvas.width,canvas.height);}; img.src=`${location.origin}/${String(path).replace(/^\//,'')}`; }
  const cards=report.tarotCards||[]; masterDrawnCards=cards; if (typeof renderCombinedDrawnCardsGrid === 'function') renderCombinedDrawnCardsGrid();
...
window.toggleTaskComplete = async function(id) {
  try { await aetheriaApi(`/recommendations/${id}/complete`, {method:'PUT'}); await syncRemoteData(); if (typeof renderRecommendationsList === 'function') renderRecommendationsList(); }
  catch(err) { alert(err.message); }
};
...
window.updateDashboardData = async function () {
  ...
  try {
    ...
    const readings = typeof getReadings === 'function' ? getReadings() : [];
    ...
    if (typeof renderGuidanceScoreGauge === 'function') renderGuidanceScoreGauge(data.average_guidance_score == null ? 0 : data.average_guidance_score);
    ...
    if (typeof addAuditLog === 'function') addAuditLog(`User analytics unavailable: ${err.message}`);
  }
};
...
async function downloadPersistedReport(type, reading) {
  ...
  try {
    ...
    if (typeof addAuditLog === 'function') addAuditLog(`Exported persisted ${type.toUpperCase()} report for ${reading.id}`);
  } catch (err) { alert(err.message); }
}
...
window.generateSpiritualConsulteReport = async function () {
  try {
    ...
    if (typeof addAuditLog === 'function') addAuditLog('Exported persisted consultant analytics workbook');
  } catch (err) { alert(err.message); }
};