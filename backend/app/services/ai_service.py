...
class AIInterpretationService:
    ...
    def gather_context(self, db: Session, user) -> Dict[str, Any]:
        ...
        preferences = (profile.preferences if profile and isinstance(profile.preferences, dict) else {}) or {}
        zodiac = preferences.get("zodiac")
    ...
    def generate_combined_master_report(self, user_name: str, goals: list, palm_data: dict, tarot_cards: list, spread_name: str, focus_intent: str, *, age_group=None, life_trend=None, personality=None, zodiac=None) -> dict:
        ...
        zodiac_prefix = f" [{zodiac} Archetype]" if zodiac else ""
        return {
            "summary": f"The combined reading for {user_name}{zodiac_prefix} links observed palm-line measurements with the {spread_name} tarot spread around {focus_intent or 'general reflection'}.",
            "composite_score": composite,
            "categories": {
                "executive_synthesis": f"For {user_name}{zodiac_prefix}, the observed palm metrics and drawn cards ({card_names}) harmonize with your natural zodiac traits to provide purposeful reflection.",
                "mindset_cognition": f"The Head Line confidence ({palm_data.get('head_line_conf', 0)}%) combined with {zodiac or 'your'} elemental mindset frames sharp decision-making habits alongside the card themes.",
                "career_ambition": f"The Fate Line confidence ({palm_data.get('fate_line_conf', 0)}%) and the tarot spread support structured milestone planning in alignment with your personal ambitions.",
                "love_relationships": f"The Heart Line confidence ({palm_data.get('heart_line_conf', 0)}%) highlights emotional depth, authentic dialogue, and mindful boundary awareness.",
                "health_vitality": f"The Life Line confidence ({palm_data.get('life_line_conf', 0)}%) acts as a symbolic reminder to honor balanced physical rhythms and adequate rejuvenation.",
                "destiny_milestones": f"Use the spread positions, palm line indicators, and your {zodiac or 'core'} purpose to identify measurable milestones within your direct influence.",
            },
            "safety_disclaimer": DISCLAIMER,
