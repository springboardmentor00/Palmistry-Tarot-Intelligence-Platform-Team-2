...
class TarotDeckService:
    ...
    @staticmethod
    def draw_spread(db: Session, spread_name: str, card_ids=None, reversed_flags=None) -> list:
        all_cards = db.query(TarotCard).order_by(TarotCard.id).all()
        if not all_cards:
            raise ValueError("Tarot deck is not seeded.")
        count_map = {"single": 1, "three": 3, "relationship": 5, "career": 5, "celtic": 10, "three card (past, present, future)": 3, "five card elemental alignment": 5, "celtic cross master spread": 10}
        normalized_name = spread_name.lower().strip()
        count = count_map.get(normalized_name, 1)
        by_id = {c.id: c for c in all_cards}
        if card_ids is not None:
            if len(card_ids) != count or len(set(card_ids)) != count:
                raise ValueError(f"The selected spread requires exactly {count} distinct cards.")
            if any(cid not in by_id for cid in card_ids):
                raise ValueError("One or more selected tarot cards do not exist.")
            drawn_cards = [by_id[cid] for cid in card_ids]
        else:
        ...
        if normalized_name == "single":
            ...
            positions = ["1. Your Energy", "2. Partner Energy", "3. Core Dynamics", "4. Main Obstacle", "5. Potential Outcome"]
        elif normalized_name == "career":
            positions = ["1. Current Position", "2. Immediate Challenge", "3. Hidden Factor", "4. Recommended Action", "5. Future Prospect"]
        elif normalized_name == "celtic":
            positions = ["1. Present Foundation", "2. Immediate Challenge", "3. Subconscious Grounding", "4. Distant History", "5. Conscious Focus", "6. Immediate Horizon", "7. Personal Power", "8. Environmental Influence", "9. Hopes & Fears", "10. Ultimate Synthesis"]
        else:
            positions = [f"Position {i + 1}" for i in range(count)]

        if reversed_flags is not None:
            if len(reversed_flags) != count:
                raise ValueError(f"The spread requires exactly {count} orientation values.")
            orientations = list(reversed_flags)
        else:
            orientations = [random.random() > 0.75 for _ in range(count)]
        return [{"card": card, "position_name": positions[idx], "is_reversed": bool(orientations[idx])} for idx, card in enumerate(drawn_cards)]
